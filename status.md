...
### 1. 인증 (카카오 + 구글 로그인)
- 로컬 회원가입/로그인 제거, `POST /auth/kakao` 카카오 인증
- `POST /auth/google` 구글 소셜 로그인 (OAuth 2.0 Authorization Code 방식)
- 첫 가입 시 `role` 선택 가능
...
| 카테고리 | 개수 |
|----------|------|
| 인증 | 4 (login, kakao, google, apply-instructor) |
| 프로그램 | 7 (CRUD + discover + my + schedules) |
| 예약 | 4 |
| 결제 | 2 |
| 출석 | 4 |
| 알림 | 4 |
...
## Sprint 18 — Dashboard Drill-down + Metrics Expansion (완료)

### 개요
대시보드 숫자 카드에서 관련 목록 페이지로 드릴다운 내비게이션 추가 + stats 기반 정적 시각화. Backend 변경 없음 (`apps/server diff = 0`).

### 주요 변경

| 항목 | 내용 |
|------|------|
| M1: Drill-down | 대시보드 5개 통계 카드에 클릭 → 관련 리스트 페이지 이동 (존재하는 endpoint/페이지만 활성화) |
| M2: Visualization | stats object 값 기반 시각 카드/차트 렌더링 (파생 지표 계산 금지) |
| URL authoritative | `router.replace` 기반 searchParams 동기화, 새로고침 시 URL 복원 |
| 에러 처리 | unified error envelope (message/code/requestId) + retry 동일 요청 재발행 |

### 변경 파일

**수정 (2개):**
- `apps/admin/src/app/page.tsx` — 대시보드 drill-down 링크 + 시각화 레이어
- `apps/admin/src/app/bulk-cancel/page.tsx` — URL sync 개선

**스펙 (1개):**
- `specs/sprint-18-admin-visibility-expansion/SPEC_SPRINT18_SSOT.md`
...
### 개요
어드민 대시보드 가시성 확장 + Bulk Cancel 관찰성 강화. Backend 변경 없음 (`apps/server diff = 0`).

### 주요 변경

| 항목 | 내용 |
|------|------|
| M1: Drill-down Expansion | Admin endpoint audit 후 유효한 drill-down만 활성화, 미존재 대상은 disabled + 사유 표시 |
| M2: Bulk Cancel Observability | 에러 패널에 code/message/requestId 항상 노출, retry 동일 요청 재발행, 로딩/갱신 상태 명확화 |
| 제약 | Backend 변경 금지, 전역 리팩토링 금지, derived metric 계산 금지 |

### 변경 파일

**수정 (2개):**
- `apps/admin/src/app/page.tsx` — drill-down 확장 + disabled 상태 처리
- `apps/admin/src/app/bulk-cancel/page.tsx` — observability 강화

**스펙 (1개):**
- `specs/sprint-19-admin-visibility-and-observability/SSOT.md`

---

## Sprint 22 — Sitemap Foundation (완료)

### 개요
Next.js Metadata Route 기반 정적 사이트맵 생성. 홈페이지(/) URL만 포함하는 최소 구현. Sprint 24에서 강사 프로필 URL 연결.

### 주요 변경

| 항목 | 내용 |
|------|------|
| `sitemap.ts` | Next.js App Router Metadata Route — 정적 URL(/) 렌더링 |
| ISR | 24시간 revalidation (`revalidate = 86400`) |
| Fallback | `NEXT_PUBLIC_SITE_URL` 미설정 시에도 빌드 미중단 |

### 변경 파일

**신규 (1개):**
- `apps/web/src/app/sitemap.ts` — 사이트맵 기초 (Sprint 24에서 강사 어댑터 연결)
...
- `apps/server pnpm test`: 16 passed, 2 failed (pre-existing reservation mock issues, Sprint 20 무관 → **M3에서 해결됨**)
...
### 검증 결과
- `apps/server pnpm build`: PASS
- `apps/server pnpm test`: 18 passed, 2 failed (pre-existing reservation mock issues, Sprint 20 무관 → **M3에서 해결됨**)
- `apps/web pnpm build`: PASS
...
`DATABASE_URL`, `NODE_ENV=production`, `JWT_SECRET`, `KAKAO_REST_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `REDIS_URL`, `PORTONE_API_SECRET`, `PORTONE_STORE_ID`, `PORTONE_CHANNEL_KEY`, `PORTONE_WEBHOOK_SECRET`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_S3_BUCKET`, `AWS_REGION`, `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY`

### 변경 파일 (1개)

- `apps/server/package.json` — `start:prod` 스크립트 수정 (`f3d6ede`)
## Repo Guardrails (verified 2026-02-16)
- Branch protection: main
- Require status checks: Web E2E Tests (.github/workflows/web-e2e.yml)
